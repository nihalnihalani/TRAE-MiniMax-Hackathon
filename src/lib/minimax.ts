...
/** Integrity score 0-100. Low score implies possible cheating/copy-pasting. Mirrors InterviewReportDialog logic. */
function calculateIntegrityScore(integrity: {
  ...
  const smallPastePenalty = Math.min(20, (integrity.pasteCount - (integrity.largePasteEvents?.length ?? 0)) * 3);
  const largePastePenalty = Math.min(35, (integrity.largePasteEvents?.length ?? 0) * 15);
  const totalPenalty = blurPenalty + smallPastePenalty + largePastePenalty;
  return Math.max(0, 100 - totalPenalty);
}

const DEFAULT_FALLBACK_REPORT: StructuredInterviewReport = {
  overallScore: 0,
  hireRecommendation: 'NO HIRE',
  executiveSummary: 'Report could not be fully generated. Evaluation may be partial due to parsing or API limits.',
  technicalEvaluation: { score: 0, summary: '—', strengths: [], weaknesses: [] },
  communicationEvaluation: { score: 0, summary: '—', strengths: [], weaknesses: [] },
  problemSolvingEvaluation: { score: 0, summary: '—', strengths: [], weaknesses: [] },
  finalFeedback: 'Please try generating the report again. If the issue persists, review the transcript and code manually.',
};
...
export async function generateInterviewReport(
  ...
  const integrity = data.integrity ?? { blurCount: 0, pasteCount: 0, largePasteEvents: [] };
  ...
  const testResultsSection = Array.isArray(data.testResults) && data.testResults.length > 0
    ? data.testResults.map((r: { testsPassed?: number; testsTotal?: number }) =>
        `passed: ${r.testsPassed ?? 0}, total: ${r.testsTotal ?? 0}`
      ).join('; ')
    : 'No tests executed';
...
`;
  ...
  try {
    ...
    if (MINIMAX_ANTHROPIC_URL && MINIMAX_API_KEY) {
      responseText = await callMiniMaxAnthropic(systemPrompt, userPrompt, "MiniMax-M2.1", 2048);
    } else {
      const fullPrompt = systemPrompt + '\n\n' + userPrompt;
      responseText = await callMiniMax([{ sender_type: "USER", sender_name: "User", text: fullPrompt }], 0.7, MODEL_NAME);
    }
    const parsed = parseJSON<StructuredInterviewReport>(responseText, DEFAULT_FALLBACK_REPORT);
    if (!parsed.success) return DEFAULT_FALLBACK_REPORT;
    return parsed.data;
  } catch (e) {
    console.error(e);
    return DEFAULT_FALLBACK_REPORT;
  }
}
