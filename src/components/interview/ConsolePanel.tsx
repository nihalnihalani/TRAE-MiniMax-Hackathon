...
export function ConsolePanel({ output }: ConsolePanelProps) {
  ...
  return (
    <div className="flex flex-col h-full bg-[#0c0c0c] border-t border-gray-800 font-mono text-sm shadow-inner">
      ...
      <pre className="flex-1 p-4 overflow-auto space-y-1 scrollbar-thin scrollbar-thumb-gray-700 scrollbar-track-transparent font-mono">
        {output.length === 0 ? (
          ...
          <div role="log" aria-live="polite" aria-label="Console output log">
            {output.map((log, i) => (
